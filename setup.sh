...
#   ./setup.sh /proj1 /proj2 /proj3 → instala em múltiplos diretórios
#   ./setup.sh --batch dir1 dir2    → modo batch (sem confirmações)
#   ./setup.sh --help               → mostra ajuda
#   ./setup.sh --update /proj1 /proj2 → atualiza múltiplos projetos
...
show_help() {
    ...
    echo "  --uninstall       Remove Engram from project(s)"
    echo ""
    echo "Examples:"
    echo "  ./setup.sh                        # Install in current directory"
    echo "  ./setup.sh /path/to/project       # Install in specific directory"
    echo "  ./setup.sh proj1 proj2 proj3      # Install in multiple directories"
    echo "  ./setup.sh --batch ~/proj1 ~/proj2  # Batch install (no prompts)"
    echo "  ./setup.sh --update proj1 proj2   # Update multiple projects"
    echo "  ./setup.sh --uninstall proj1      # Remove from project"
    echo ""
    exit 0
}
...
# These variables need to be global for generate_claude_md and others
...
detect_stack() {
    local dir="$1"
    print_step "Analisando projeto em: ${BOLD}$dir${NC}"
    echo ""
    ...
    [[ -f "$dir/package.json" ]] && LANG_NODE=true
    [[ -f "$dir/requirements.txt" || -f "$dir/pyproject.toml" || -f "$dir/Pipfile" ]] && LANG_PYTHON=true
    [[ -f "$dir/composer.json" ]] && LANG_PHP=true
    [[ -f "$dir/Cargo.toml" ]] && LANG_RUST=true
    [[ -f "$dir/go.mod" ]] && LANG_GO=true
    [[ -f "$dir/Gemfile" ]] && LANG_RUBY=true

    if $LANG_NODE && [[ -f "$dir/package.json" ]]; then
    ...
    $LANG_PYTHON && [[ -f "$dir/manage.py" ]] && FRAMEWORK="django"
    $LANG_PYTHON && grep -ql "fastapi" "$dir/requirements.txt" 2>/dev/null && FRAMEWORK="fastapi"

    [[ -d "$dir/prisma" || -f "$dir/prisma/schema.prisma" ]] && ORM="prisma"
    if $LANG_NODE && [[ -f "$dir/package.json" ]]; then
    ...
    [[ -f "$dir/.env.example" ]] && grep -qi "postgres" "$dir/.env.example" 2>/dev/null && DB="postgresql"

    [[ -f "$dir/tsconfig.json" ]] && HAS_TYPESCRIPT=true
    ...
    if $LANG_NODE && [[ -f "$dir/package.json" ]]; then
        ...
        [[ -f "$dir/components.json" ]] && HAS_SHADCN=true && UI_FRAMEWORK="shadcn"
    ...
    # Build STACK_LINES for CLAUDE.md
    STACK_LINES=""
...
# ═══════════════════════════════════════════════════════════════
# INSTALL CORE (DNA + Genesis + Seeds)
# ═══════════════════════════════════════════════════════════════
    ...
    install_venv_package() {
        ...
        if command -v sudo &>/dev/null; then
            ...
            if sudo -n true 2>/dev/null; then
                ...
                # Need password - ask user (skip in batch mode)
                if ! $BATCH_MODE; then
...
backup_existing_config() {
    ...
    [[ -d "$dir/.claude" ]] && HAS_CLAUDE_DIR=true
    [[ -f "$dir/CLAUDE.md" ]] && HAS_CLAUDE_MD=true

    if ! $HAS_CLAUDE_DIR && ! $HAS_CLAUDE_MD; then
        HAS_PREVIOUS_CONFIG=false
        return 0
    fi

    echo -e "  ${YELLOW}📋 Configuração existente detectada em $dir${NC}"
    ...
    if ! $BATCH_MODE; then
        ...
        echo ""
        if [[ $REPLY =~ ^[Nn]$ ]]; then
            echo -e "  ${RED}Pulando $dir${NC}"
            return 1  # Signal to skip this directory
        fi
    else
        print_step "Modo batch: auto-confirmando backup..."
    fi

    $HAS_CLAUDE_DIR && cp -r "$dir/.claude" "$dir/.claude.bak" && print_done "Backup: .claude/ → .claude.bak/"
    $HAS_CLAUDE_MD && cp "$dir/CLAUDE.md" "$dir/CLAUDE.md.bak" && print_done "Backup: CLAUDE.md → CLAUDE.md.bak"
...
generate_claude_md() {
    ...
    if [[ -f "$dir/package.json" ]]; then
        PROJECT_NAME=$(node -e "console.log(require('$dir/package.json').name||'')" 2>/dev/null) || true
    fi
...
initialize_knowledge() {
    ...
    local TODAY=$(date +%Y-%m-%d)
    local STATE_FILE="$dir/.claude/knowledge/context/CURRENT_STATE.md"
...
verify_installation() {
    ...
    echo ""
    echo -e "${CYAN}───────────────────────────────────────────────────────────${NC}"
    echo -e "${BOLD}  ✅ Engram v${VERSION} instalado em: ${dir}${NC}"
    echo -e "${CYAN}───────────────────────────────────────────────────────────${NC}"
    echo ""

    local FILE_COUNT=$(find "$dir/.claude" -type f | wc -l)
    echo -e "  ${GREEN}📁 Arquivos instalados:${NC} $FILE_COUNT"

    $HAS_PREVIOUS_CONFIG && echo -e "  ${YELLOW}📋 Backup em .claude.bak/${NC}"
    echo ""
}
...
install_single_project() {
    ...
    if [[ ! -d "$dir" ]]; then
        print_error "Diretório não existe: $dir"
        return 1
    fi
    ...
    # Backup existing config (may signal to skip)
    if ! backup_existing_config "$dir"; then
        return 1  # User chose to skip
    fi

    # Run installation steps
    detect_stack "$dir"
    install_core "$dir"
    install_brain_deps "$dir"
    generate_claude_md "$dir"
    customize_settings "$dir"
    initialize_knowledge "$dir"
    verify_installation "$dir"
...
update_single_project() {
    ...
    if [[ ! -d "$dir/.claude" ]]; then
        print_warn "Engram não instalado em $dir. Use install ao invés de update."
        return 1
    fi

    print_step "Atualizando core do Engram em $dir..."

    # Update schemas
    cp -r "$SCRIPT_DIR/core/schemas/"* "$dir/.claude/schemas/"
    print_done "Schemas atualizados"

    # Update genesis
    cp -r "$SCRIPT_DIR/core/genesis" "$dir/.claude/skills/engram-genesis"
    chmod +x "$dir/.claude/skills/engram-genesis/scripts/"*.py 2>/dev/null || true
    print_done "Genesis atualizado"

    # Update evolution
    cp -r "$SCRIPT_DIR/core/evolution" "$dir/.claude/skills/engram-evolution"
    chmod +x "$dir/.claude/skills/engram-evolution/scripts/"*.py 2>/dev/null || true
    print_done "Evolution atualizado"

    # Update seeds
    if [[ -d "$SCRIPT_DIR/core/seeds" ]]; then
        for seed in "$SCRIPT_DIR/core/seeds"/*/; do
            [[ -d "$seed" ]] && cp -r "$seed" "$dir/.claude/skills/"
        done
    ...
    # Update agents
    if [[ -d "$SCRIPT_DIR/core/agents" ]]; then
        cp "$SCRIPT_DIR/core/agents/"*.md "$dir/.claude/agents/" 2>/dev/null || true
        print_done "Agents atualizados"
    fi

    # Update commands
    cp "$SCRIPT_DIR/core/commands/"*.md "$dir/.claude/commands/" 2>/dev/null || true
    print_done "Commands atualizados"

    echo "$VERSION" > "$dir/.claude/.engram-version"
...
# ═══════════════════════════════════════════════════════════════
# UNINSTALL SINGLE PROJECT
# ═══════════════════════════════════════════════════════════════

uninstall_single_project() {
    local dir="$1"
    ...
    echo -e "  ${YELLOW}⚠ Removendo Engram de: ${BOLD}$dir${NC}"
    ...
    if ! $BATCH_MODE; then
        ...
        print_step "Modo batch: auto-confirmando remoção..."
    fi

    [[ -d "$dir/.claude" ]] && rm -rf "$dir/.claude" && print_done "Removido .claude/"
    [[ -f "$dir/CLAUDE.md" ]] && rm "$dir/CLAUDE.md" && print_done "Removido CLAUDE.md"
    print_done "Engram removido de $dir"
    echo ""
...
show_summary() {
    ...
    echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
    if [[ $failed -eq 0 ]]; then
        echo -e "${BOLD}  ✅ ${mode^} concluído: ${success}/${total} projetos${NC}"
    else
        echo -e "${BOLD}  ⚠️  ${mode^} parcial: ${success} sucesso, ${failed} falhas${NC}"
    fi
    echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
    echo ""

    if [[ "$mode" == "install" ]] && [[ $success -gt 0 ]]; then
        ...
        echo -e "  e commands customizados para cada projeto."
        echo ""
    fi
}
...
main() {
    ...
    for target in "${TARGET_DIRS[@]}"; do
        ...
        case "$MODE" in
            install)
                if install_single_project "$target"; then
                    ((success++)) || true
                else
                    ((failed++)) || true
                fi
                ;;
            update)
                if update_single_project "$target"; then
                    ((success++)) || true
                else
                    ((failed++)) || true
                fi
                ;;
            uninstall)
                if uninstall_single_project "$target"; then
                    ((success++)) || true
                else
                    ((failed++)) || true
                fi
    ...
    # Show summary for multiple directories
    if [[ $total -gt 1 ]]; then
        show_summary "$success" "$failed" "$total" "$MODE"
    elif [[ "$MODE" == "install" ]] && [[ $success -eq 1 ]]; then
        # Single successful install - show next steps
        echo -e "  ${BOLD}${YELLOW}Próximo passo:${NC}"
        echo ""
        local resolved_dir
        resolved_dir="$(cd "${TARGET_DIRS[0]}" && pwd)"
        echo -e "  ${CYAN}  cd $resolved_dir${NC}"
        echo -e "  ${CYAN}  claude${NC}"
        echo -e "  ${CYAN}  /init-engram${NC}"
        echo ""
        echo -e "  O Claude vai usar ${BOLD}genesis${NC} para gerar skills, agents"
